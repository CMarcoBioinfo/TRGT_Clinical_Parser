...
from scripts.bio.motif_structure import motif_interruptions
...
def process_repeats(r):
    """Nettoyage et interruptions internes aux motifs."""
    r["Répétition1"] = remove_zero_repeats(r["Répétition1"])
    r["Répétition2"] = remove_zero_repeats(r["Répétition2"])
    
    r["Répétition1"] = motif_interruptions(r["Répétition1"], r["Segmentation1"])
    r["Répétition2"] = motif_interruptions(r["Répétition2"], r["Segmentation2"])
...
def process_plots(r, base_dir, prefix, sample_name):
    ...
    trid = r["TRID"]
    r["Plots"] = get_available_plots(trid, base_dir, prefix, sample_name)
